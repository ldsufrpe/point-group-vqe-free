...
behind the paper's six canonical experiments (**287 runs**). It is not the authors' working
...
…and likewise for the other five directories.
...
`run.py` accepts a molecule key (`h2o`, `nh3`, `ch4`, `all`) and `--smoke`. The figures shipped
in `experiments/*/figures/` are byte-identical to the ones printed in the paper (md5 verified).
...
- **Unitary ansatz, operator counts (E1).** Against the Abelian baseline, NH₃ goes 163 → 147 and
  CH₄ 146 → 146. These count *distinct elementary excitation operators*, not two-qubit gates.
  **Circuit depth was never measured**, and no device-level resource claim is supported here.
...
per-run record, and the six of them sum to 287 runs. `env/system_info.json` records the machine,
