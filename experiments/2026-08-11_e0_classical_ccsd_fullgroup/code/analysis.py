...
def main():
    ...
    bx.text(-0.45, 4.0, "chemical accuracy", ha="left", va="bottom", fontsize=7,
            color=X.OKABE_ITO[0])
    bx.set_yscale("log")
    # The floor at 1e-13 must sit ABOVE the axis lower limit, or the markers
    # clamped onto it are cut in half by the bottom spine.
    bx.set_ylim(2e-14, 300)
    ...
    manifest = {
        "amplitude_classes": {
            "panels": ["upper (no letter drawn): surviving CCSD amplitude classes per molecule under three filters",
                       "lower (no letter drawn): absolute change in CCSD correlation energy caused by each filter, "
            ...
            "elements": [
                ...
                {"artist": "numeric label above every bar", "encodes":
                 "the exact class count", "visual": "black text, 7 pt", "panel": "(a)"},
                ...
                {"artist": "legend", "encodes": "the three filters",
                 "visual": "upper left of (a), three columns, no frame, 7 pt", "panel": "(a)"},
            ],
